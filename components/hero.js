...
// 60 frames × 70 ms/frame = 4 200 px of scroll distance to play the full animation
const SCROLL_DISTANCE = 60 * 70;
...
export default function Hero({ city }) {
  ...
  useEffect(() => {
    const video = wrapperRef.current && videoRef.current ? videoRef.current : null;
  ...
  return (
    /*
     * Outer wrapper is tall enough to give SCROLL_DISTANCE px of scroll room
     * beyond the viewport height. The inner section is sticky so it stays
     * pinned while the user scrolls through that distance.
     */
    <div
      ref={wrapperRef}
      className="bg-gradient-to-br from-emerald-50 via-white to-blue-50"
      style={{ height: `calc(100vh + ${SCROLL_DISTANCE}px)` }}
    >
      <section
        id="home"
        className="sticky top-0 h-screen flex items-center pt-20 bg-gradient-to-br from-emerald-50 via-white to-blue-50 overflow-hidden"
      >
        <div className="container-custom section-padding">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            {/* Right Video — scroll-driven spray animation */}
